...
typedef struct MyAVPacketList {
  AVPacket *pkt;
  // 两个 serial
  // 通过比较匹配来丢弃无效的缓存帧，什么情况会导致队列的缓存帧无效？跳转播放时间点的时候。
  // 例如此时此刻，PacketQueue 队列里面缓存了 8 个帧，但是这 8 个帧都 第30分钟
  // 才开始播放的，如果你通过 ➔ 按键前进到 第35分钟 的位置播放，那队列的 8
  // 个缓存帧就无效了，需要丢弃。 由于每次跳转播放时间点， PacketQueue::serial
  // 都会 +1 ，而 MyAVPacketList::serial 的值还是原来的，两个 serial
  // 不一样，就会丢弃帧。
  int serial;//来源于PackQueue
} MyAVPacketList;

typedef struct PacketQueue {
  AVFifoBuffer *pkt_list; // 存储的是MyAVPacketList数据
  int nb_packets;         // 数据包的数量
  int size;               // 缓存的数据大小，等于AVPacket + AVPacket->size
  int64_t duration;       // 所有数据包的总时长
  int abort_request;      // 如果为1.audio_thread和video_thread线程会退出
  int serial; // 队列的序列号(可以看作最新的序列号)，用于同步和顺序控制.每次跳转播放时间点就会加1
  SDL_mutex *mutex;
  SDL_cond *cond;
} PacketQueue;
...
typedef struct Clock {
  ...
  //通过 pts 字段减去系统时间得到的，所以实际运行的时候 pts_drift 是一个很大的负数
  //即pts_drift = pts - 之前记录的系统时间
  //set_clock_at函数中有写
...
// 环形队列,rindex和windex会回滚
typedef struct FrameQueue {
  Frame queue[FRAME_QUEUE_SIZE];
  // 解码得到部分数据，windex往前移动，播放部分数据，rindex往前移动
  int rindex; // 当前的读索引位置
  int windex;
  int size;     // 不是内存大小，而是个数，表示当前队列缓存了多少个Frame
  int max_size; // 队列最多缓存多少个Frame
  // 保存在队列里面有什么用呢？用视频流举例，当SDL窗口变小的时候，ffplay
  // 可以取上一帧Frame，重新渲染 texture 来适应缩小后的窗口大小。
  int keep_last; // 播放之后是否保存上一帧在队列里面不销毁.在代码里写死的
  // 说白了，rindex指向的是播放了的一帧，加上rindex_shown才是要播放的帧
  int rindex_shown; // 配合keeplast使用
  SDL_mutex *mutex;
  SDL_cond *cond;    // 解码线程和播放线程通信
  PacketQueue *pktq; // 来源，FrameQueue的数据从哪个PackQueue来的
} FrameQueue;
...
//serial: packet->decoder->frame
//序列号是为了快进快退功能实现的
typedef struct Decoder {
  AVPacket *pkt;         // 要进行解码的 AVPacket，也是要发送给解码器的 AVPacket
  PacketQueue *queue;    // AVPacket 队列
  AVCodecContext *avctx; // 解码器实例
  int pkt_serial;        // 序列号,记录的是上一次用来解码的AVPacket的序列号
  int finished; // 已完成的时候，finished 等于上面的 pkt_serial。当 buffersink
                // 输出 EOF 的时候就是已完成。
  int packet_pending; // 代表上一个 AVPacket
                      // 已经从队列取出来了，但是未发送成功给解码器。未发生成功的会保留在第一个字段
                      // pkt 里面，下次会直接发送，不从队列取。
  SDL_cond *empty_queue_cond; // 条件变量，AVPacket
                              // 队列已经没有数据的时候会激活这个条件变量。
  int64_t start_pts;          // 流的第一帧的pts
  AVRational start_pts_tb;    // 流的第一帧的pts的时间基
  // 解码出来的 AVFrame 的 pts 有些是 AV_NOPTS_VALUE，这时候就需要next_pts来纠正
...
typedef struct VideoState {
  ...
  Clock audclk; // 记录音频流的目前的播放时刻
  Clock vidclk; // 记录视频流的目前的播放时刻
  // 外部时钟，取第一帧 音频 或 视频的 pts 作为
  // 起始时间，然后随着物理时间的消逝增长，所以是物理时间的当前时刻。到底是以音频的第一帧，还是视频的第一帧？取决于
  // av_read_frame() 函数第一次读到的是音频还是视频。
  Clock extclk;
  ...
  // 有 3
  // 种同步方式，以音频时钟为准，以视频时钟为准，以外部时钟为准。默认方式是以音频时钟为准。
  ...
  //窗口正在显示的帧 的播放时刻，就是说这帧是何时开始播放的，从何时开始显示到窗口上的
  double frame_timer;
  double frame_last_returned_time;
  //滤镜容器处理上一帧所花的时间(预估值).那么处理这一帧也差不多是这个时间
  ...
  // read_thread() 线程在以下两种情况会进入休眠 10ms。
  ...
  for (;;) {
    if (d->queue->serial == d->pkt_serial) {
      do {
        ...
        case AVMEDIA_TYPE_VIDEO:
          ret = avcodec_receive_frame(d->avctx, frame);//首次必然等于EAGAIN，因为还没send
          if (ret >= 0) {
            if (decoder_reorder_pts == -1) {
              //best_effort_timestamp由libavcodec自动计算，可以直接使用
    ...
    do {
      if (d->queue->nb_packets == 0)
        SDL_CondSignal(d->empty_queue_cond);//唤醒read_thread读数据
      if (d->packet_pending) {
        //如果为1，则上次发送失败了，现在pkt里是有值的，直接发给解码器即可
        d->packet_pending = 0;
      } else {
...
// 第一次读的是0， 第二次rindex_shown变为1，rindex还是0，读的就是1,以此类推
// 读的是下一帧
...
static void frame_queue_next(FrameQueue *f) {
  if (f->keep_last && !f->rindex_shown) {
    // 在第一次进入的时候rindex_shown为0,rindex会不移动直接退出
    // 下次读数据的时候直接用读索引读到的就是上一帧的内容
    f->rindex_shown = 1;
    return;
  }
...
static double compute_target_delay(double delay, VideoState *is) {
  ...
  if (get_master_sync_type(is) != AV_SYNC_VIDEO_MASTER) {
    ...
    diff = get_clock(&is->vidclk) - get_master_clock(is);//单位秒
    /* skip or repeat frame. We take into account the
       delay to compute the threshold. I still don't know
       if it is the best guess */
    //音视频不同步是正常的,只要控制在阈值内即可,所以要先计算出阈值
    ...
    //如果超过max_frame_duration,说明不同步的时间太大,直接摆烂不管
    if (!isnan(diff) && fabs(diff) < is->max_frame_duration) {
      if (diff <= -sync_threshold)//视频慢了, 通常将delay置为0
...
static void video_refresh(void *opaque, double *remaining_time) {
  ...
  retry:
    if (frame_queue_nb_remaining(&is->pictq) == 0) {
      ...
      //这样做可以把失效的 Frame 通通丢弃。因为快进快退的时候，会导致队列里面缓存的 Frame 失效
      if (vp->serial != is->videoq.serial) {
        frame_queue_next(&is->pictq);
        goto retry;
      }

      //如果上一帧和当前帧的序列号不一样，说明当前帧是上一帧的下一帧，所以就把当前帧的时间戳设置为上一帧的时间戳
      //一般是进行了快进快退操作
      if (lastvp->serial != vp->serial)
        is->frame_timer = av_gettime_relative() / 1000000.0;

      //暂停状态下是有可能调整窗口的,这就需要上一帧来重新渲染sdl
      if (is->paused)
        goto display;

      /* compute nominal last_duration */
      last_duration = vp_duration(is, lastvp, vp);
      //last_duration代表当前帧本来显示需要多长时间, delay代表实际应该显示多长时间
      //比如视频比音频快了 0.05s 的时候，那 delay 就会从 0.04 变成 0.08，翻倍了，
      //拉长当前视频帧的播放时间来等待音频流追上来。
  ...
  display:
    /* display picture */
    //display_disable可以通过命令行参数控制, 默认是0
    //is->force_refresh 变量有两种情况会置为 1，一是当下一帧可以播放的时候，二是当窗口大小产生变化的时候
    //rindex_shown 的初始值是 0。只有在插入第一帧到 FrameQueue 的时候， rindex_shown 变量才会变成 1
    if (!display_disable && is->force_refresh && is->show_mode == SHOW_MODE_VIDEO && is->pictq.rindex_shown)
      //取的是上一帧视频,把视频帧AVFrame数据渲染到SDL_Texture(纹理)上面
      video_display(is);
  }
  is->force_refresh = 0;
  //4. 日志
...
static int get_video_frame(VideoState *is, AVFrame *frame) {
    int got_picture; // 变量，用于指示是否成功解码出图像
    ...
    // 检查是否成功解码出图像
    if (got_picture) {
        double dpts = NAN; // 初始化解码后的显示时间戳（PTS）为 NaN

        // 如果帧有有效的 PTS，使用流的时间基准将其转换为双精度浮点数
        if (frame->pts != AV_NOPTS_VALUE)
        ...
        // 根据 framedrop 设置和同步类型检查是否需要丢弃帧
        // c99没有布尔值,framedrop默认为-1,只要是非值都为真
        if (framedrop > 0 || (framedrop && get_master_sync_type(is) != AV_SYNC_VIDEO_MASTER)) {
            // 如果帧有有效的 PTS，计算其与主时钟的差值
            if (frame->pts != AV_NOPTS_VALUE) {
    ...
    return got_picture; // 返回是否成功解码出图像（1）或未解码（0）
}
...
static int configure_filtergraph(AVFilterGraph *graph, const char *filtergraph,
  ...
  if (filtergraph) {
    ...
    outputs->name = av_strdup("in"); // 动态分配，可修改
...
// force_output_format ，代表是否强制把 buffersink 出口滤镜的音频帧采样等信息
// 设置为 跟 is->audio_tgt 一样
static int configure_audio_filters(VideoState *is, const char *afilters,
  ...
  // 释放滤镜容器，第一次之后可能就不是null了，所以需要释放
  avfilter_graph_free(&is->agraph);
  if (!(is->agraph = avfilter_graph_alloc()))
    return AVERROR(ENOMEM);
  is->agraph->nb_threads = filter_nbthreads; // 0为自动选择线程数量
  ...
  // 无论MP4文件里面的音频格式是怎样的，他都会转成 AV_SAMPLE_FMT_S16
  // 格式丢给SDL播放
...
//解码PacketQueue队列中的AVPacket，解码出来的AVFrame丢给入口滤镜，
//从出口滤镜把AVFrame读出来，再插入FrameQueue队列
...
//解码 PacketQueue 队列里面的 AVPacket 的，解码出来 AVFrame，
//然后丢给入口滤镜，再从出口滤镜把 AVFrame 读出来，再插入 FrameQueue 队列
static int video_thread(void *arg) {
  ...
  int last_w = 0;//上一次解码出来的AVFrame的宽度
  int last_h = 0;//上一次解码出来的AVFrame的高度
  enum AVPixelFormat last_format = -2;  //上一次解码出来的AVFrame的像素格式
  int last_serial = -1;//上一次解码出来的AVFrame的序列号
  int last_vfilter_idx = 0;//上一次使用的视频滤镜索引
#endif
      ...
      if (is->audio_diff_avg_count < AUDIO_DIFF_AVG_NB) {
       ...
       //加权平均数
        ...
        if (fabs(avg_diff) >= is->audio_diff_threshold) {
          //diff * is->audio_src.freq算出在diff时间内增加的样本数量
          wanted_nb_samples = nb_samples + (int)(diff * is->audio_src.freq);
          min_nb_samples = ((nb_samples * (100 - SAMPLE_CORRECTION_PERCENT_MAX) / 100));
          max_nb_samples = ((nb_samples * (100 + SAMPLE_CORRECTION_PERCENT_MAX) / 100));
          //音频的连续性很强,样本的调整幅度不能超过10%,否则耳朵能够听出来
 ...
 * value.
 */
//该函数和解码无关，真正解码的函数是decoder_decode_frame
//从FrameQueue中读取Frame，并将其传输到音频输出设备，同时处理音量和同步问题
static int audio_decode_frame(VideoState *is) {
  ...
  do {
#if defined(_WIN32)
    //在一些老旧设备中，解码线程会慢于播放线程，需要等待最多1/2的回调时间
    ...
    if (wanted_nb_samples != af->frame->nb_samples) {
      ...
      if (swr_set_compensation(is->swr_ctx,
                               (wanted_nb_samples - af->frame->nb_samples) *
                                   //采样率单位转换, audio_tgt.freq 为打开的喇叭的采样率, 可能跟frame不一样
                                   is->audio_tgt.freq / af->frame->sample_rate,
                               wanted_nb_samples * 
                                   is->audio_tgt.freq / af->frame->sample_rate) < 0) {
...
//实时解码音频帧并将其传输到音频输出设备，同时处理音量和同步问题
...
static int audio_open(void *opaque, int64_t wanted_channel_layout,
  ...
  while (
    ...
    //如果channel是7，将会切换为6声道，具体映射见next数组
    wanted_spec.channels = next_nb_channels[FFMIN(7, wanted_spec.channels)];
    //如果channel已经全部尝试完毕，开始尝试改变采样率
...
static int stream_component_open(VideoState *is, int stream_index) {
  ...
  is->eof = 0;
  // 流属性设置为不丢弃,这样就可以通过av_read_frame读取数据
  // 在read_thread设置为全部丢弃，这样就实现只读取最好的流
...
#if CONFIG_AVFILTER
  {
    ...
    // 需要从 out_audio_filter 读取 AVFrame
    ...
    // src 指的是 is->swr_ctx
    // 重采样实例的源头，也就是当需要进行重采样(不一定要重采样)的时候，
    // 要输入给is->swr_ctx 的原始音频格式就是 is->audio_src
...
static int read_thread(void *arg) {
  ...
  int err, i, ret;
  int st_index[AVMEDIA_TYPE_NB]; // 存储最好的视频流、音频流等。。
  AVPacket *pkt = NULL;
  ...
  // 控制播放器要不要停止播放(在播放网络流的时候，网络卡顿，av_read_frame可能要阻塞8秒才返回)
  // 如果想关闭播放器，av_read_frame内部会定时执行该回调函数，若返回1，av_read_frame就不会再阻塞
  ic->interrupt_callback.callback = decode_interrupt_cb;
  ic->interrupt_callback.opaque = is;
  // 在数字视频广播或多媒体文件中，可能存在多个流（如多个音频轨道或多个视频轨道）。通过扫描所有
  // PMT，程序能够获取到所有可用流的信息，确保用户能够选择和播放所需的内容。
  ...
  // 当命令行写错的时候或某些原因会导致一些option没使用，这就是在检查哪些option没有使用
  ...
  if (st_index[AVMEDIA_TYPE_VIDEO] >= 0) {
    ...
    // 显示的时候用 sar 这个比例拉伸 width 跟 height
    // 作为显示窗口，图像播放就不会扭曲了。sar 在大部分情况都是 1:1
  ...
  // 主要任务:不断从容器中读取AVPacket然后丢进PacketQueue
  for (;;) {
    if (is->abort_request)
      break;
    // 暂停只对支持暂停的某些网络流媒体协议有效(播放本地文件无效)
...
static VideoState *stream_open(const char *filename, AVInputFormat *iformat) {
  ...
  // 拷贝字符串s的一个副本，由函数返回值返回，这个副本有自己的内存空间，和s没有关联
  ...
  is->muted = 0;
  is->av_sync_type = av_sync_type; // 同步方式以音频为准
  // 启动read_thread线程
...
static void refresh_loop_wait_event(VideoState *is, SDL_Event *event) {
  ...
  while (!SDL_PeepEvents(event, 1, SDL_GETEVENT, SDL_FIRSTEVENT, SDL_LASTEVENT)) {
    //如果光标没有被隐藏，并且自上次显示光标以来的时间超过了 CURSOR_HIDE_DELAY，则隐藏光标
...
int main(int argc, char **argv) {
  ...
  // 设置动态库加载规则，这是一个安全函数，在 Windows 系统，默认会从当前目录加载
  // DLL，这容易被攻击。这个函数就是把当前目录的路径从加载规则里面去掉，里面调的是
  // SetDllDirectory("")。
  init_dynload();

  // 设置日志标志，使libavutil的日志系统跳过重复的信息
  av_log_set_flags(AV_LOG_SKIP_REPEATED);
  // 调用parse_loglevel函数，该函数根据命令行参数argc和argv以及options结构体设置日志级别
